...
3. Check the journal for warnings or stack traces:
   ```
   ssh root@vps.bounnit-aymane.fr 'journalctl -u discordbot -n 50 --no-pager'
   ```
...
2. Manual deploy from your laptop, equivalent to what CI does:
   ```
   ssh -i ~/.ssh/sirrmizan_ci botdiscord@vps.bounnit-aymane.fr
   ```
...
2. If something serious happened (manual edit gone wrong, partial write),
   ...
   ```
   ssh root@vps.bounnit-aymane.fr
   ls -la /root/backups/
...
```
ssh root@vps.bounnit-aymane.fr 'systemctl stop discordbot'
```

The watchdog also will not bring it back if you also disable the cron:
```
ssh root@vps.bounnit-aymane.fr 'systemctl stop discordbot && crontab -l | grep -v sirrmizan-heartbeat-check | crontab -'
```
