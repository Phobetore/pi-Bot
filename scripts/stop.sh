#!/usr/bin/env bash
# Stop pi-Bot. Sends SIGTERM and waits up to TIMEOUT seconds for a clean
# shutdown (during which the bot flushes state to disk). Falls back to
# SIGKILL only if that fails.
#
# Environment overrides:
#   PIDFILE : pidfile location (default: .run/pi-bot.pid)
...
if [[ ! -f "$PIDFILE" ]]; then
    echo "no pidfile at $PIDFILE — is pi-Bot running?" >&2
    exit 1
fi
...
echo "stopping pi-Bot (pid $PID, waiting up to ${TIMEOUT}s)..."
kill -TERM "$PID"
